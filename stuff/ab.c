...
//     LibreOffice (http://www.documentfoundation.org/download/) charting.
//
...
#define NUM_REQUESTS "10000"  // million is a good value
...
#define ITER       1 // number of iterations (3: worse, average, best)
...
int main(int argc, char *argv[])
{
   ...
   for(i = FROM; i <= TO; i += STEP)
   {
      ...
      sprintf(str, "httperf --server=" IP " --port=" PORT " "
               ...
               , i?i:1, i?i:1);
#elif defined LIGHTY_WEIGHTTP
      sprintf(str, "weighttp -n " NUM_REQUESTS " -c %d -t %u %s "
